...
def test():
    loss_list= []
    acc_list =[]
    test_dataloader = get_dataloader(train=False,batch_size=TEST_BATCH_SIZE) # train=false 选择测试集
    for idx,(input,target) in enumerate(test_dataloader):
        with torch.no_grad():
            output = model(input) # 调用模型,使用测试集的input 得到预测值 output[batch_size,10] target:[batch_size]
            cur_loss = F.nll_loss(output,target) # 得到损失
            loss_list.append(cur_loss) # 计算平均
            pred = output.max(dim=-1)[-1] # 取每一行的最大值的下标
            cur_acc = pred.eq(target).float().mean() # 将下标和目标值相计算得到准确率
            acc_list.append(cur_acc) # 计算平均 计算平均方法:np.mean()
            # 计算准确率
